...
import com.music.scraper.model.{BandPage, Disbanded, Formed, Member}
...
object Scraper {
  ...
  private def getBandName(document: Document): String = document >> element("h1.artist_name_hdr") >> text
  ...
  private def getHeaders(element: Element): List[String] = element >> elementList(".info_hdr") >> text

  private def getDetails(element: Element): List[Element] = element >> elementList(".info_content")
